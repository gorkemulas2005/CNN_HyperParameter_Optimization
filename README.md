...
`dense_units` değeri VGG16 ve ResNet50 sınıflandırıcı başlıklarında doğrudan kullanılır. Custom CNN ve Proposed CNN/Custom V2 model kurucuları bu parametreyi almadığı için, bu iki modelde `dense_units` arama uzayında kayıtlı görünse de final mimariyi değiştirmez.
...
Bayesyen TPE kodunda `dense_units` tüm modeller için önerilir; ancak Custom CNN ve Proposed CNN/Custom V2 kurulumlarında kullanılmaz. Bu nedenle `dense_units` etkisi yalnızca VGG16 ve ResNet50 classifier başlıkları için deneysel olarak geçerlidir.
...
## Mimari Hiperparametre Kapsamı

Bu çalışmada hiperparametre optimizasyonu tüm katman tiplerini serbestçe tasarlayan bir neural architecture search olarak uygulanmamıştır. Arama uzayı, kodda açıkça tanımlanan parametrelerle sınırlıdır.

| Model | Conv katman sayısı optimize edildi mi? | Conv kanal sayısı optimize edildi mi? | Kernel size optimize edildi mi? | Pooling optimize edildi mi? | Açıklama |
| --- | --- | --- | --- | --- | --- |
| VGG16 | Hayır | Hayır | Hayır | Hayır | ImageNet omurgası sabittir; yalnızca classifier başlığı ve eğitim hiperparametreleri seçilir. |
| ResNet50 | Hayır | Hayır | Hayır | Hayır | ImageNet omurgası sabittir; yalnızca classifier başlığı ve eğitim hiperparametreleri seçilir. |
...
Başlangıç hipotezi, transfer öğrenme modellerinde Bayesyen TPE'nin, sıfırdan eğitilen modellerde ise GA'nın daha avantajlı olabileceği yönündedir. Test sonuçları bu hipotezi tamamen doğrulamamaktadır. ResNet50 transfer öğrenme koşulunda Bayesyen TPE daha iyi sonuç vermiştir; ancak VGG16'da iki yöntem neredeyse eşdeğerdir. Sıfırdan eğitilen Custom CNN koşulunda GA açık biçimde daha iyi sonuç üretmiştir; fakat yine sıfırdan eğitilen Proposed CNN koşulunda Bayesyen TPE daha yüksek accuracy ve macro F1 değerlerine ulaşmıştır. Bu nedenle bulgu, “hazır modellerde Bayesyen, sıfırdan modellerde GA” biçiminde kesin bir kural olarak değil, mimari özelinde değişen bir eğilim olarak raporlanmalıdır.
...
- `dense_units` arama uzayında ortak parametre olarak görünür; fakat Custom CNN ve Proposed CNN/Custom V2 mimarilerinde kullanılmaz.
