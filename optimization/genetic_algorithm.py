"""
optimization/genetic_algorithm.py  —  DAGM 2007
-------------------------------------------------
DEAP tabanlı GA — num_classes parametresi dışarıdan alınır.
"""
...
def run_ga(
    ...
    print(f"\n{'='*55}")
    print(f" GA Optimizasyonu — Model: {model_name.upper()}")
    print(f" Pop: {pop_size} | Nesil: {n_generations} | Epoch/Birey: {epochs_per_eval}")
    print(f"{'='*55}")

    gene_len = CUSTOM_LEN if model_name == "custom" else COMMON_LEN
    ...
    print(f"\n  [GA] En iyi val_acc={best_val_acc:.4f} | Süre={elapsed/60:.1f} dk")
    return best_params, best_val_acc, elapsed, logbook
